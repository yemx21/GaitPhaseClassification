...
#define NO_COLOR -1
#define DEFAULT_RESIZE_FACTOR 20
...
namespace p1d
{
	...
	*/
	...
	public:
		...
		*/
		bool RunPersistence(const std::vector<float>& InputData, int resizefactor = -1)
		{
			Data = InputData;
			Init(resizefactor);
	...
	protected:
		...
		*/
		void Init(int resizefactor)
		{
			...
			int vectorSize = (int)(Data.size() / (resizefactor==-1 ? DEFAULT_RESIZE_FACTOR : resizefactor)) + 1; //starting reserved size >= 1 at least
